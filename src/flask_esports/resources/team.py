...
class Team:
    ...
    def __init__(
        self, team_id: SourceId, name: str, tag: str, logo: str, region: str
    ) -> None:
        self.id = team_id
        self.name = name
        self.tag = tag
    ...
    def add_staff(self, staff_id: int, display_name: str, role: Role):
        self.current_staff.append(
            {"id": staff_id, "name": display_name, "role": str(role)}
        )

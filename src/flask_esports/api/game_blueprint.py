...
from ..app.resources import Match, Player
...
class GameBlueprint:
    ...
    def create_get_player_matches_endpoint(game: str, router: GameRouter) -> callable:
        ...
        def get_player_matches(player_id: int) -> Response:
            player_matches: list[Match] = router.get_player_matches(
    ...
    def create_get_team_matches_endpoint(game: str, router: GameRouter) -> callable:
        ...
        def get_team_matches(team_id: int):
            matches = router.get_team_matches(
    ...
    def create_get_team_players_endpoint(
        game: str, router: GameRouter, current: bool
    ) -> callable:
        @require_int("team_id", GameBlueprint._invalid_id_msg("team_id"))
    ...
    def __init__(self, router: GameRouter, game: str, import_name: str):
        ...
        self._bp.add_url_rule(
            "/player/<player_id>",
            "get_player",
            GameBlueprint.create_get_player_endpoint(game, router),
        )
        self._bp.add_url_rule(
            "/player/<player_id>/matches",
            "get_player_matches",
            GameBlueprint.create_get_player_matches_endpoint(game, router),
        )
        self._bp.add_url_rule(
            "/player/<player_id>/teams",
            "get_player_teams",
            GameBlueprint.create_get_player_teams_endpoint(game, router),
        )
        ...
        self._bp.add_url_rule(
            "/team/<team_id>",
            "get_team",
            GameBlueprint.create_get_team_endpoint(game, router),
        )
        self._bp.add_url_rule(
            "/team/<team_id>/matches",
            "get_team_matches",
            GameBlueprint.create_get_team_matches_endpoint(game, router),
        )
        self._bp.add_url_rule(
            "/team/<team_id>/players",
            "get_team_players",
            GameBlueprint.create_get_team_players_endpoint(game, router, current=False),
        )
        self._bp.add_url_rule(
            "/team/<team_id>/players/current",
            "get_team_players_current",
            GameBlueprint.create_get_team_players_endpoint(game, router, current=True),
        )
        ...
        self._bp.add_url_rule(
            "/match/<match_id>",
            "get_match",
            GameBlueprint.create_get_match_endpoint(game, router),
        )
        ...
        self._bp.add_url_rule(
            "/event/<event_id>",
            "get_event",
            GameBlueprint.create_get_event_endpoint(game, router),
        )
        self._bp.add_url_rule(
            "/event/<event_id>/matches",
            "get_event_matches",
            GameBlueprint.create_get_event_matches_endpoint(game, router),
        )
        self._bp.add_url_rule(
            "/event/<event_id>/teams",
            "get_event_teams",
            GameBlueprint.create_get_event_teams_endpoint(game, router),
        )
